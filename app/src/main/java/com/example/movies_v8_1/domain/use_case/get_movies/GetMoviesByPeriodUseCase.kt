...
import com.example.movies_v8_1.common.toPremiereEntity
...
class GetMoviesByPeriodUseCase @Inject constructor(
    private val repository: MovieRepository
) {
    operator fun invoke(period: PremieresPeriod): Flow<Resource<List<MovieModel>>> = flow {
        try {
            emit(Resource.Loading())
            val movies = repository.getMoviesByPeriod(period).items.map { it.toMovieModel() }
            repository.writeMoviesToPremieresTable(movies.map { it.toPremiereEntity() } )
